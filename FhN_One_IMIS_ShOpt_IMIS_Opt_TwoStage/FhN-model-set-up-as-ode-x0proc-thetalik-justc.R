...
######################################################################################
make.FHN<-function(){
    fhn.fun <- function(times, y, par, more) {
    ...
    fhn.fun.ode <- function(times, y, par) {
        #print("fn.ode")
      if (length(par)==1){
        p=c(.2,.2,par,.05)
      }else{
    ...
    fhn.dfdx <- function(times, y, par, more) {
    ...
    fhn.dfdp <- function(times, y, par, more) {
        #print("fhn.dfdp")
		p=c(.2,.2,par,.05)
		names(p)=c("a","b","c","sd")
		r = array(0, c(dim(y), 1))
        dimnames(r) = list(NULL, c("V","R"), c("c"))
        r[, "V", "c"] = (y[, "V"] - y[, "V"]^3/3 + y[, "R"])
        r[, "R", "c"] = ((y[, "V"] - p["a"] + p["b"] * y[, "R"])/(p["c"]^2))
		
		return(r)
    }
    fhn.d2fdx2 <- function(times, y, par, more) {
        #print("dfdx2")
		p=c(.2,.2,par,.05)
		names(p)=c("a","b","c","sd")
	...
	fhn.d2fdxdp <- function(times, y, par, more) {
        #print("d2fdxdp")
		p=c(.2,.2,par,.05)
		names(p)=c("a","b","c","sd")		
        r = array(0, c(dim(y), 2, 1))
        dimnames(r) = list(NULL, c("V","R"), c("V","R"), c("c"))
        r[, "V", "V", "c"] = 1 - y[, "V"]^2
        r[, "V", "R", "c"] = 1
        r[, "R", "V", "c"] = 1/p["c"]^2
        r[, "R", "R", "c"] = p["b"]/p["c"]^2
    ...
    d2SSEproc.dc2 <- function(coefs, bvals, pars, more) {
        ...
        for (i in 1:dim(devals)[2]) {
            ...
            for (j in 1:dim(devals)[2]) {
                H[[i]][[j]] = t(bvals$bvals) %*% diag(H1[, i, j]) %*% bvals$bvals - 2 * t(bvals$dbvals) %*% diag(H2[, i, j] * weights[, i]) %*% bvals$bvals - 	2 * t(bvals$bvals) %*% diag(H2[, j, i] * weights[,j]) %*% bvals$dbvals
