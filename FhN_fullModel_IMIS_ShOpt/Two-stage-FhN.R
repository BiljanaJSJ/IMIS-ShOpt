...
d.g<- -sum(arg2*du)+g*sum(arg2^2)-sum(dv*arg1)/(g^2)-sum(arg1^2)/(g^3)
                    #d(obj)/d(gamma)
c(d.a,d.b,d.g)
}
...
der.fhn.justc<-function(b,du,dv,u,v)
{
    ...
    #d.a<- -sum(dv)/g-sum(arg1)/(g^2)        #d(obj)/d(alpha)
    #d.b<- sum(v*dv)/g+sum(arg1*v)/(g^2)     #d(obj)/d(beta)
    return( -sum(arg2*du)+g*sum(arg2^2)-sum(dv*arg1)/(g^2)-sum(arg1^2)/(g^3))
    #d(obj)/d(gamma)
}
...
jac.fhn.justc<-function(b,du,dv,u,v)
{
    ...
    arg2<- u+v-u^3/3
    J<-matrix(NA,1,1)
    J<- sum(arg2^2)+2*sum(dv*arg1)/(g^3)+3*sum(arg1^2)/(g^4)
    return(J)
}
...
simex.fun.justc<-function(par,u.obs,v.obs,s1,s2,B=100)
{
    ...
    for (s in 1:m.lam)
    {
        ...
        for (b in 1:B)
        {
            ...
            derv.u.b=lokerns(times,u.obs.lam,x.out=times,deriv=1)$est
            derv.v.b=lokerns(times,v.obs.lam,x.out=times,deriv=1)$est
            beta.B[b,]<-neq(b=par[3],der.fhn.justc,jac.fhn.justc,du=derv.u.b,dv=derv.v.b,
            u=u.obs.lam,v=v.obs.lam)$b
        } # end loop b
        beta.lambda[s,]<-apply(beta.B,2,mean,na.rm=T)
        var.lambda[s,]<-apply(beta.B,2,var,na.rm=T)
...
#############NOTE: I'm actually already estimating the residual measurement error as another parameter.
...
twostage.optim.fhn.justc<-function(pars,data=datamatrix,times=timepoints){
    ...
    #par[4]=var.estimate=diag(var(resids))
    #par[5]=v.ker[1]
    #par[6]=u.ker[1]
    ...
    #ker.est<-neq(b=pars,der.fhn.justc,jac.fhn.justc,du=du.ker,dv=dv.ker,u=u.ker,v=v.ker)$b
    #cal.arg[j,]<-ker.est
    #lin.est<-linear.fhn.fit(du=du.ker,dv=dv.ker,u=out[,2],v=out[,3])
    #lin.arg[j,]<-lin.est
    return(simex.fun.justc(parms,data[,1],data[,2],s1=0.1, s2=0.1,B=100))
    #    return(c(simex.fun(pars,data[,1],data[,2],s1=0.1, s2=0.1,B=100),par[4:6]))
}
...
twostage.optim.fhn<-function(data=datamatrix,times=timepoints,pars){
n=dim(data)[1]
fac.arg=max(1.2,n^(2/35)*(log(n))^(-1/4))
h.dT=lokerns(times,data[,1],x.out=times,deriv=1)$bandwidth*fac.arg
h.T=lokerns(times,data[,1],x.out=times,deriv=0)$bandwidth*fac.arg
...
v.ker=v.ker.arg$mx
    #######################Estimates for other parameters
     resids = cbind(u.ker-data[,1],v.ker-data[,2])
    pars[4:5]=var.estimate=diag(var(resids))
    pars[6]=v.ker[1]
    pars[7]=u.ker[1]
    sigej=c(u.ker.arg$sig_e, v.ker.arg$sig_e)
    parms=pars
names(pars[6:7])=c("V0","R0")
...
pars[1:3]=simex.fun(par=pars[1:3],u.obs=data[,1],v.obs=data[,2],s1=0.1, s2=0.1,B=100)
