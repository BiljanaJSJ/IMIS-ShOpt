  ...
  for (k in 1:number_k){
    ...
    if (k==1){
      ...
      while(length(which_remain)>0 & i<=D/3){
        ...
        if (is.vector(X_all)){	
          ...
          res = foreach(index = runIndex, .export=c('neglogpriorx0')) %dopar%{
          ...
          if (all(runIndex== c(1,2)))  {optimizer  = res[[1]];optimizer2 = res[[2]];optimizer3=NULL}
          if (all(runIndex==c(2,3)))   {optimizer2  = res[[1]];optimizer3 = res[[2]];optimizer=NULL}
          if (all(runIndex== c(1,3)))  {optimizer  = res[[1]];optimizer3 = res[[2]];optimizer2=NULL}
          ...
          if (all(runIndex== c(1,2)))  {sigma_all[[ls1]]  = solve(optimizer$hessian);sigma_all[[ls2]]  = solve(optimizer2$hessian)}
          if (all(runIndex==c(2,3)))   {sigma_all[[ls1]]  = solve(optimizer2$hessian);sigma_all[[ls2]]  = solve(optimizer3$hessian)}
          if (all(runIndex== c(1,3)))  {sigma_all[[ls1]]  = solve(optimizer$hessian);sigma_all[[ls2]]  = solve(optimizer3$hessian)}
          
          
          
          
          # if (1 %in% runIndex) {sigma_all[[ls1]]  = solve(optimizer$hessian)}
          # if (2 %in% runIndex) {sigma_all[[ls2]]  = solve(optimizer2$hessian)}
          # if (3 %in% runIndex) {sigma_all[[ls3]]  = solve(optimizer3$hessian)}
          
          
          # if (1 %in% runIndex) {X_k = c(X_k, rnorm(B, center_all[ls1], sqrt(sigma_all[[ls1]])) )}	                        # Draw new samples based on old optimizer
          # if (2 %in% runIndex) {X_k = c(X_k, rnorm(B, center_all[ls2], sqrt(sigma_all[[ls2]])) )}	                        # Draw new samples based on smoother optimizer
          # if (3 %in% runIndex) {X_k = c(X_k, rnorm(B, center_all[ls3], sqrt(sigma_all[[ls3]])) )}	                                                # Draw new samples based on 2-stage optimizer
          
          X_k = c(X_k, rnorm(B, center_all[ls1], sqrt(sigma_all[[ls1]])) )
          X_k = c(X_k, rnorm(B, center_all[ls2], sqrt(sigma_all[[ls2]])) )
        ...
        if (is.matrix(X_all)){
          ...
          res = foreach(index=runIndex) %dopar%{
          ...
          if (1 %in% runIndex) {r1=res[[1]]}
          if (2 %in% runIndex) {r2=res[[2]]}
          if (3 %in% runIndex) {r3=res[[3]]}
          
          if (1 %in% runIndex) {optimizer   = list(par=r1,hessian=hessian(posterior,r1,data=data),value = posterior(r1,data=data));names(optimizer$par)=parnames}else{optimizer=NULL}
          if (2 %in% runIndex) {optimizer2  = list(par=r2,hessian=hessian(posterior,r2,data=data),value = posterior(r1,data=data));names(optimizer2$par)=parnames}else{optimizer2=NULL}
          if (3 %in% runIndex) {optimizer3  = list(par=r3,hessian=hessian(posterior,r3,data=data),value = posterior(r1,data=data));names(optimizer3$par)=parnames}else{optimizer3=NULL}
          ...
          if (1 %in% runIndex) {ls1=length(sigma_all)+1}else{ls1=NULL}
          if (2 %in% runIndex) {ls2=length(sigma_all)+2}else{ls2=NULL}
          if (3 %in% runIndex) {ls3=length(sigma_all)+3}else{ls3=NULL}
          
          if (1 %in% runIndex)      {if (min(eigen(optimizer$hessian)$values)>0)    	sigma_all[[ls1]] = solve(optimizer$hessian)	}					# the covariance of new samples
          if (2 %in% runIndex)      {if (min(eigen(optimizer2$hessian)$values)>0)    sigma_all[[ls2]] = solve(optimizer2$hessian)	}											# the covariance of new samples using the smoothing based optimizer
          if (3 %in% runIndex)      {if (min(eigen(optimizer3$hessian)$values)>0)    sigma_all[[ls3]] = solve(optimizer3$hessian)	}											# the covariance of new samples using the smoothing based optimizer
          ...
          if (1 %in% runIndex){X_k = rbind(X_k, rmvnorm(B, optimizer$par, sigma_all[[ls1]]) )}			# Draw new samples
          # print("X_k from optimizer")
          # print(X_k)
          ############# END OF NOTE
          
          if (2 %in% runIndex){X_k = rbind(X_k, rmvnorm(B, optimizer2$par, sigma_all[[ls2]]) )}			# Draw new samples using the smoothing based optimizer.
          
          
          if (3 %in% runIndex){X_k = rbind(X_k, rmvnorm(B, optimizer3$par, sigma_all[[ls3]]) )}		# Draw new samples using the smoothing based optimizer.
        ...
        if (all(runIndex==1:2)){		
          ...
          if (is.matrix(X_all))	distance_remain = apply(abs(X_all[which_remain,]-optimizer2$par),1,sum)
          if (is.vector(X_all))	distance_remain = abs(X_all[which_remain]-optimizer2$par)
          label_dist = sort(distance_remain, decreasing = FALSE, index=TRUE)
          which_exclude = union( which_exclude, which_remain[label_dist$ix[1:floor(size_remain/D)]])
          which_remain = setdiff(which_remain, which_exclude)
        }
        ...
        if (all(runIndex==2:3)){
          if(length(which_remain)>0){
            ...
            if (is.matrix(X_all))	distance_remain = apply(abs(X_all[which_remain,]-optimizer3$par),1,sum)
            if (is.vector(X_all))	distance_remain = abs(X_all[which_remain]-optimizer3$par)
            label_dist = sort(distance_remain, decreasing = FALSE, index=TRUE)
            which_exclude = union( which_exclude, which_remain[label_dist$ix[1:floor(size_remain/D)]])
            which_remain = setdiff(which_remain, which_exclude)
          } }
        ...
        if (all(runIndex==1:3)){
          if(length(which_remain)>0){
            ...
            # next do this with the smoother optimizer
            if (is.matrix(X_all))	distance_remain = apply(abs(X_all[which_remain,]-optimizer$par),1,sum)
            if (is.vector(X_all))	distance_remain = abs(X_all[which_remain]-optimizer1$par)
            label_dist = sort(distance_remain, decreasing = FALSE, index=TRUE)
            which_exclude = union( which_exclude, which_remain[label_dist$ix[1:floor(size_remain/D)]])
            which_remain = setdiff(which_remain, which_exclude)
    ...
    if (k>1 | option.opt==0){
      ...
      if (is.vector(X_all)){
        ...
        if (length(X_all[which_var]-X_imp) == length(Weights_var)){
          Sig2 = (X_all[which_var]-X_imp)^2 %*% Weights_var}else{Sig2=0}
