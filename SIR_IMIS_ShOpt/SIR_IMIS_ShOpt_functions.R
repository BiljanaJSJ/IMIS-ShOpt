...
###################################################################################
#SIRlogpost    evaluate joint log posterior  P(alpha,beta, I0/  Y)
...
###################################################################################
#logprior  evaluate log prior (beta,alpha,I0)
#input:        pars      - a vector of (beta,alpha)
#              I         - numerical solution for I
#              priorpars - prior parameters for alpha, beta and I0
...
NLSStep =function(IniPar=NULL,times,dat,cov_prior,R){
  ShotGun=R
  ...
  if (is.null(IniPar)){
    #(IniPar= c(b=abs(rnorm(1,0,.005)),a=abs(rnorm(1,0,.05)), I0=abs(rnorm(1,5,5*(1-5/N))) ))
    (IniPar= c(b=abs(rnorm(1,0,.005)),a=abs(rnorm(1,0,.05))))
    # }else{
    #   IniPar=IniPar[c(1,2,4)]
  }
  for (i in (1:ShotGun)){
    #print(i)
    (OptimOut         = optimx(par=IniPar[1:2],I0=i,fn=minimize.f,dat=dat,N=N,times=times,method='Nelder-Mead',control=list(maxit=1000)))#,control=list(maxit =10000)))
    (parameters[i,]   = c(OptimOut[[1]],OptimOut[[2]],i,OptimOut$value))
    
    calcHess[[i]]    = hessian(function(x) (-1)*SIRlogpost_alphaBeta(pars=c(x[1],x[2]),y=dat,I0=i,N=N,times=times),parameters[i,1:2])
    
    
    if (!(is.singular.matrix(calcHess[[i]]))){
   ...
   if (all(calcHess[[i]]==0)){ calcHess[[i]]=cov_prior}
    }
    
  
    
    if (min(eigen(calcHess[[i]])$values) <=0) {
      warning('Hessian is not positive definite!')             
      eigen.values = eigen(calcHess[[i]])$values
      eigen.values[which(eigen.values < 0)] = 0
      calcHess[[i]]=calcHess[[i]]+abs( min(eigen(calcHess[[i]])$values))*diag(dim(calcHess[[i]])[1])*1.01
    }
    print('Third place')   
    print(i)
    
    # if (min(eigen(calcHess[[i]])$values)<=0){			
    #   eig = eigen(calcHess[[i]])
    ...
    #   #   
    #   #   calcHess[[i]]  = tryCatch({
    #   #     round(solve(hess_new1 + diag(1/diag(Sig2_global)) ),6)
    #   #   }, warning = function(w) {
    #   #     msg('Singular matrix')
    #   #   }, error = function(e) {
    #   #     round(Sig2_global,6)
  ...
  #  calcHess[[i]]=calcHess[[i]]+diag(1/diag(cov_prior))
...
symbolicDeriv.function = function(vec,deriv.level=0,a=NULL,b=NULL,I0=NULL,
  ...
  #if (a>0 & b>0 & I0>0){
  if (a>0 && b>0 ){
    f=expression(  SSE(pars=c(b,a,I0),times=times,dat=dat,rez=rez) )
  }else{f=99999999}
  #print("NLS")
  ...
  if(deriv.level==0){# just evaluate the function
    ...
    # output = eval(f,list(b=b, a=a))
    
  }
  # I could make this code more efficient by only computing the derivatives that we need.
  return(output)
}
...
Rhat=function(thetas,dat,N=25,times,rez=NULL){
...
runOptimizers=function(IniPars=NULL,times,dat,runOptim,D=3,R=10,cov_prior=NULL,N=261){
  ...
  if (!is.na(runOptim[1])){
    ...
    NLSstep_out= NLSStep(IniPar=IniPars[1:2],times=times,dat=dat,cov_prior=cov_prior,R=R)
...
runIMIS_ShOpt=function(cl,N0=3*1000,D=3,R=10,B=10000,J=30000,niter=100,priorpars=c(1,1,1,1,N,5/N),N,dat,times,par_true=c(0.0062,0.098,256,5)){
  
  #initial stage: sample parameters from the prior
  #library(sqldf)
  #theta=list()
  ...
  #library(deSolve)
   ...
   # out=get(load(paste('samples',1,'.RData',sep='')))
   # theta_all=out$theta_all;theta_max=out$theta_max;wts_old=out$wts;
   # center=out$center;sigma_max=out$sigma_max;H_k=out$H_k;
   # norm_all=out$norm_all;log_lik_all=out$log_lik_all;prior_all=out$prior_all;
   # cov_prior_global=out$cov_prior_global
   # cov_prior_global = cov(theta_all[1:N0,c(1,2)])
  # theta_k=theta_all[(N0+1):nrow(theta_all),]
  ...
  for (k in (1:niter)) {
    ...
    if (k == 1) 
      norm_all = prior_all
    if (k > 1) 
      norm_all=log(apply(rbind(exp(prior_all) * N0/B, H_k), 2, sum)/(N0/B + R*D + (k - 2)))
    ...
    if (k==1 && D>0){
      ...
      center=matrix(NA,R*D,ncol(theta_all))
      
      d=1
      theta_k=excluded=NULL
      theta_max=matrix(NA,D,ncol(theta_all))
      #H_k=matrix(0,R*D,N0+B*R*D)
      for (j in (1:D)){ 
        if (j==1){
          indmax=which( wts[[k]]==max( wts[[k]]))  
        }else{
          indmax=which( wts[[k]]==max( wts[[k]][-excluded]))
        }
        
        
        if (length(indmax)>1){indmax=sample(indmax,1)}
        theta_max[j,]=theta_all[indmax,]
        #Stage 2 run the optimizers
        
        
        optimRezults             = runOptimizers(IniPars=theta_max[j,],times=times,dat=dat,D=D,R=R,runOptim=c(1,1,1),cov_prior=cov_prior_global)
        #IniPars=theta_max[j,];times=times;dat=dat;D=D;R=R;runOptim=c(1,1,1);cov_prior=cov_prior_global
        ...
        print(optimRezults$parameters)
        
        for (i in (1:R)){
          #loop over results for each of the D points within the optimization routine
          #remove them from theta0 points that are close to the mode  
          #based on Mahalanobis distance
          A                        = theta_all[,c(1,2)]
          m                        = optimRezults$parameters[[1]][i,c(1,2)]
          #sigma_max[[d]]           = solve(optimRezults$hessian[[1]][[i]])
          sigma_max[[d]]           = optimRezults$hessian[[1]][[i]]
          
          
          
          center[d,]               = c(m[1:2],N-i,i)
          
          mah_distance             = mahalanobis(A, m, sigma_max[[d]])
          
          label_dist               = sort(mah_distance, decreasing = FALSE, index = TRUE)
          indB                     = label_dist$ix[1:(N0/(R*D))]
          excluded                 = c(excluded,indB)
          
          excluded                 = unique(excluded)
          #repopulate here with Gaussian centered around the mode and hessian
          #draw B points for each theta_opt
          
          repop                    = rmvnorm(B,center[d,c(1,2)]  , sigma=sigma_max[[d]])
          #repop[,3]                = abs(round(repop[,3]))
          repop                    = cbind(repop,i)
          theta_k                  = rbind(theta_k,cbind(repop[,1:2],N-repop[,3],repop[,3]))
      ...
      H_k=matrix(0,R*D,N0 + R*D * B)
      d=1
      for (j in (1:D)){ #loop over results from different optimization routines
        for (i in (1:R)){ 
          H_k[d,]         = dmvnorm(theta_all[,c(1,2)],center[d,][c(1,2)], sigma=sigma_max[[d]])
          d               = d+1
        }
      }     
      
      out                               = list(theta_all=theta_all,theta_k=theta_k,theta_max=theta_max,wts=wts[[k]],k=k,center=center,sigma_max=sigma_max,H_k=H_k,norm_all=norm_all,log_lik_all=log_lik_all,prior_all=prior_all,cov_prior_global=cov_prior_global) 
      save(out,file=paste('samples',k,'.RData',sep=''))
    ...
    if (k > 1 | D == 0) {
      ...
      indmax                   = which(wts[[k]]==max(wts[[k]]))
      if (length(indmax)>1){indmax=sample(indmax,1)}
      theta_max                = rbind(theta_max,theta_all[indmax,])
      print(theta_max)
      ...
      center                   = rbind(center,c(m[1:2],N-theta_all[indmax,4],theta_all[indmax,4]))
      
      
      
      distance_all             = mahalanobis(A, m, diag(diag(cov_prior_global)))
      
      label_dist               = sort(distance_all, decreasing = FALSE, index = TRUE)
      indB                     = setdiff(label_dist$ix[1:B],which(is.nan(A[,2])))
      
      wts_cov                  = wts[[k]][indB]+(1/length(wts[[k]]))
      wts_cov                  = wts_cov/sum(wts_cov)
      cov_weighted             = cov.wt(x=A[indB,],wt=wts_cov,cor=FALSE,center=m,method='unbias')
      sigma_max[[R*D + k - 1]] = cov_weighted$cov
      repop_k                  = rmvnorm(B,m,sigma=cov_weighted$cov)
      repop_k                  = cbind(repop_k,theta_all[indmax,4])                    
      #repop_k[,3]              = round(repop_k[,3])
      
      
      repop_k_4                = cbind(repop_k[,1:2],N-repop_k[,3],repop_k[,3])
    ...
    #   }else{
    H_k_new                                           = matrix(0,R*D+k-1,nrow(theta_all))
    H_k_new[1:(R*D + k - 2),1:(dim(theta_all)[1]-B)]  = H_k
    
    H_k_new[R*D + k - 1,]                             = dmvnorm(theta_all[,c(1,2)],center[R*D+k-1,c(1,2)], sigma=sigma_max[[R*D + k - 1]])
    
    for (j in (1:(R*D + k - 2))) {
      H_k_new[j, 
              (dim(theta_all)[1] - B + 1):dim(theta_all)[1]] = dmvnorm(theta_k[,c(1,2)], center[j,c(1,2)], sigma_max[[j]])
    }
    H_k                              = H_k_new
    }
  ...
  out                               = list(stat_all=stat_all,theta=theta,theta_all=theta_all,theta_max=theta_max,wts=wts[[k]],k=k,center=center,sigma_max=sigma_max,H_k=H_k,norm_all=norm_all,log_lik_all=log_lik_all,prior_all=prior_all,cov_prior_global=cov_prior_global) 
  save(out,file='samples.RData')
  return(out)
